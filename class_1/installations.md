...
## Install git
Set up GitHub on your computer if not already. Most macs already come with it. Type “man git” in your terminal to check, if you have already have git installed. If not, go to https://git-scm.com/book/en/v2/Getting-Started-Installing-Git and install git. There are platform specific instructions on this page.

## Clone the class repo
Run `git clone git@github.com:aartivnkt/intro-to-python.git` to clone the repo for the class. Before each class, remember to pull the latest git changes using `git pull origin main`
...
### JupyterLab test
Type `jupyter lab` in the `intro-to-python` directory where the cloned git repo for the class lives. Start working on `worksheet_1a.ipynb` to experiment with your first jupyterlab notebook that uses system Python 3.12.7. Close notebook and move onto the next section once complete. 

### Python 3.12.7 test 
This test is to ensure python 3.12.7 kernel can be selected in JupyerLab, under the kernel dropdown. Close any JupyterLab sessions if you have them open. Type `jupyter lab` again and work on `worksheet_1b.ipynb` this time, choosing "Python 3.12.7" kernel from the top right

### virtualenv test
Activate virtualenv. Run `worksheet_1c.ipynb` in virtualenv to experiment with JupyterLab!
